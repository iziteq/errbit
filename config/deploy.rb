...
namespace :errbit do
  ...
  task :symlink_configs do
    ...
    run("ln -nfs #{shared_configs}/newrelic.yml #{release_configs}/newrelic.yml")
    run("ln -nfs #{shared_configs}/daemon_config.yml #{release_configs}/daemon_config.yml")
    run("ln -nfs #{shared_configs}/mongoid.yml #{release_configs}/mongoid.yml")
...
namespace :unicorn do
  ...
  task :restart, :roles => :app, :except => { :no_release => true } do
    run "sudo /etc/init.d/errbit restart"
  end

  desc 'Stop unicorn'
  task :stop, :roles => :app, :except => { :no_release => true } do
    run "sudo /etc/init.d/errbit stop"
  end

  desc 'Start unicorn'
  task :start, :roles => :app, :except => { :no_release => true } do
    run "sudo /etc/init.d/errbit start"
  end
end
